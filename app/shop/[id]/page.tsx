...
export default function ProductPage() {
  ...
  useEffect(() => {
    ...
    fetch("/api/products")
      ...
      .then((products: Product[]) => {
        ...
        setTimeout(() => setLoading(false), 1000); // Altijd minimaal 1 seconden loading
      });
  }, [id]);
  ...
  if (!product) {
    return (
      <main className="flex flex-col items-center justify-center bg-[#F8F8F8] py-16 px-4 min-h-[60vh]">
  ...
  return (
    <main className="bg-[#F8F8F8] flex flex-col py-8 px-2 sm:px-6">
      {/* Breadcrumb */}
