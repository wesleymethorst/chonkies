...
export default function SaleProductPage() {
    ...
    return (
      <main className="bg-[#F8F8F8] min-h-screen flex flex-col">
        ...
        <div className="flex flex-col items-center justify-center flex-1 py-16 px-4 min-h-[60vh]">
          ...
          <Link href="/sale" className="text-[#FF5CA2] underline">Terug naar sale</Link>
        </div>
        <Footer />
      </main>
    );
  ...
  const salePrice =
    typeof product.sale_price === "number"
      ? product.sale_price
      : typeof product.sale_price === "string" && product.sale_price !== "null" && product.sale_price !== ""
        ...
        <div className="flex-1 flex flex-col py-8 px-2 sm:px-6">
          ...
          <nav className="max-w-6xl mx-auto mb-6 px-2 text-sm" aria-label="Breadcrumb">
            <ol className="flex items-center gap-2 text-[#FF5CA2]">
              ...
              <li>
                <Link href={`/sale?category=${encodeURIComponent(product.category)}`} className="hover:underline">
                  {product.category}
                </Link>
                <span className="mx-1 text-gray-400">/</span>
              </li>
              <li className="text-[#3B5FFF] font-semibold truncate max-w-[120px]">{product.display_name}</li>
            </ol>
          </nav>
          <div className="max-w-6xl w-full mx-auto flex flex-col md:flex-row gap-10 md:gap-16 bg-white/90 rounded-3xl shadow-2xl border-2 border-[#FF5CA2] p-6 md:p-12">
            ...
            <div className="flex-1 flex flex-col justify-center order-3 max-w-xl mx-auto md:mx-0">
              <span className="uppercase text-xs font-bold text-[#FF5CA2] mb-2 tracking-widest">{product.category}</span>
              <h1 className="font-luckiest-guy text-4xl sm:text-5xl text-[#FF5CA2] mb-3">{product.display_name}</h1>
              <div className="flex items-center gap-3 mb-4">
                {salePrice !== null ? (
                  <>
                    <span className="text-[#FF5CA2] font-bold text-3xl">€{salePrice.toFixed(2)}</span>
                    <span className="line-through text-gray-400 text-xl">€{price.toFixed(2)}</span>
                  </>
                ) : (
                  <span className="text-[#FF5CA2] font-bold text-3xl">€{price.toFixed(2)}</span>
                )}
              </div>
              <div className="mb-6">
              ...
              <div className="flex items-center gap-4 mb-8">
                <button
                  className="bg-[#FF5CA2] hover:bg-[#e94b91] text-white font-poppins font-bold py-3 px-8 rounded-lg transition-all shadow text-lg"
                  onClick={() => addToCart({ ...product, id: String(product.id), quantity: 1 })}
                >
                  Voeg toe aan winkelwagen
                </button>
                <Link href="/sale" className="text-[#FF5CA2] underline text-base">← Terug naar sale</Link>
              </div>
